...
#    $Revision: 50464 $
#    $Locker:  $
#    $Date: 2013-12-16 20:00:41 +0000 (Mon, 16 Dec 2013) $
#    $State: Exp $
#
################################################################################
...
use CGIUtils qw(trim json_decode json_encode);
use DBCORE;
...
sub save_process {
    ...
    if (defined($defc)) {
        ...
        unless ($renamed) {
            ...
            $retval = $db->sp_call('processes', 'define', $def->{'type'}, $def->{'name'}, '');
            unless (defined($retval)) {
                _set_error(__LINE__, $db->error());
                return 0;
            }
        ...
        if ($def->{'class'} ne $defc->{'class'}) {
            ...
            unless ($pfc_id) {
                ...
                if ($def->{'class'} eq $def->{'name'} && $defc->{'class'} eq $defc->{'name'} && $def->{'category'} eq $defc->{'category'}) {
                    ...
                    $retval = $db->sp_call('process_family_classes', 'define', $def->{'category'}, $def->{'class'}, ''); # No description.
                    unless (defined($retval)) {
                        _set_error(__LINE__, $db->error());
                        return 0;
                    }
...
sub delete_process_definition {
    ...
    ", $pid, $pid, $pid, $pfcid, $pid, $pid, $pid, $pid, $pid, $pfcid)));
