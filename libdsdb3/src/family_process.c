    ...
    const char *name)
{
    ...
    if (class) {
        fam_proc->class = msngr_copy_string(class);
        if (!fam_proc->class) {
...
void dsdb_free_family_process(FamProc *fam_proc)
{
    if (fam_proc) {
        ...
        if (fam_proc->class)    free(fam_proc->class);
    ...
    FamProc    ***fam_procs)
{
    ...
    *fam_procs = (FamProc **)NULL;
    ...
    status = dsdbog_inquire_family_processes(dsdb->dbconn,
        category, class, site, facility, proc_type, proc_name, &dbres);
