...
static int _update_next_begin_time_file(time_t begin_time)
{
    ...
    if (!_dsproc_create_timestamp(begin_time, timestamp)) {
        return(0);
    }
...
/* TRANS QC UPDATE
 *
 * We are disabling the new transformation QC bits about missing bin
 * width/alignment values for now... When these are added back in
 * the bit 10 and 11 descriptions need to be added as well.
 *
 * These need to be set based on a compile time or SVN date so we can
 * make the library backward compatible with released software. */

QC_ESTIMATED_INPUT_BIN  = 0;
QC_ESTIMATED_OUTPUT_BIN = 0;
    ...
    time_t *interval_end)
{
    ...
    *interval_end   = 0;
    ...
    if (_DSProc->interval_begin == 0) {
        ...
        _DSProc->interval_begin  = _DSProc->period_begin;
    }
    else {
    ...
    if (!_DSProc->cmd_line_begin) {
        ...
        if (!_update_next_begin_time_file(_DSProc->interval_begin)) {
            return(0);
        }
    }
    else {
        ...
        if (status > 0 && _DSProc->interval_begin > last_begin_time) {
