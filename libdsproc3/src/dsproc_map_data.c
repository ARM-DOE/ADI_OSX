...
*    $Revision: 60673 $
*    $Author: ermold $
*    $Date: 2015-03-05 18:38:22 +0000 (Thu, 05 Mar 2015) $
*
********************************************************************************
*
    ...
    CDSGroup *dataset)
{
    ...
    if (in->time_var) {
        ...
        if (_MapBeginTime) map_begin_time = _MapBeginTime;
        else               map_begin_time = _DSProc->interval_begin;

        if (_MapEndTime)   map_end_time   = _MapEndTime;
        else               map_end_time   = _DSProc->interval_end;

        ref_timeval.tv_sec  = map_begin_time;
        ref_timeval.tv_usec = 0;

        in->sample_start = cds_find_timeval_index(
            in->ntimes, in->sample_times, ref_timeval, CDS_GTEQ);

        ref_timeval.tv_sec = map_end_time;
    ...
    if (out_qc_var) {
        ...
        if (in_qc_var) {
            ...
            if (consolidate_trans_qc) {
                ...
                status = dsproc_consolidate_var_qc(
                    in_qc_var, bad_mask, out_qc_var, bad_flag, ind_flag, 1);
            }
            else {
