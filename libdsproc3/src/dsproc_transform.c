...
/* TRANS QC UPDATE
 *
 * These will need to be added back in based on a compile time or SVN date
 * so we can make the library backward compatible with released software.
 */
//static TransAtts gTransQCAtts2[] =
//{
//    { "bit_10_description", "QC_ESTIMATED_INPUT_BIN:  Bin size and/or alignment not given for input datastream, and therefore was estimated." },
//    { "bit_10_assessment",  "Indeterminate" },
//    { "bit_11_description", "QC_ESTIMATED_OUTPUT_BIN:  Bin size and/or alignment not given for output datastream, and therefore was estimated." },
    ...
    *ind_flag = 0;
    ...
    for (ai = 0; ai < natts; ++ai) {
        ...
        if (strncmp(att->value.cp,
            "Transformation could not finish", 31) == 0) {

            *bad_flag |= 1 << (bit_num-1);
        }
        else if (strncmp(att->value.cp,
            "Transformation resulted in an indeterminate outcome", 51) == 0) {
 ...
 *  @return
 *    -  1 if successful
 *    -  0 if alignment and/or width are not defined,
 *       or the width is 0
...
static int _dsproc_create_trans_bounds_var(CDSVar *trans_coord_var)
{
    ...
    if (!cds_get_dim(trans_group, "bound")) {
        ...
        if (!cds_define_dim(trans_group, "bound", 2, 0)) {
            ...
            ERROR( DSPROC_LIB_NAME,
                "Could not define transformation dimension: %s->bound\n",
                trans_group->name);
    ...
    trans_bounds_var = cds_define_var(trans_group,
        bounds_var_name, trans_coord_var->type, 2, dim_names);
    ...
    *att->value.cp = toupper(*att->value.cp);
    ...
    length = trans_coord_var->dims[0]->length;

    if (!cds_alloc_var_data(trans_bounds_var, 0, length)) {
        goto TRANSFORM_ERROR;
    }
 ...
 *  @param ret_var         - pointer to the retrieved CDSVar
 *  @param dim_index       - index of the ret_var dimension to create the
    ...
    **********************************************************************/
    ...
    if (dim_values) {
        ...
        DEBUG_LV1( DSPROC_LIB_NAME,
            ...
            " - using values specified in the transformation parameters file\n",
            trans_coordsys->name, dim_name);
...
/* END BDE OBS UPDATE */
    ...
    **********************************************************************/
    ...
    for (dim_index = 0; dim_index < ret_var->ndims; dim_index++) {
        ...
        /* Get the dimensions and coordinate variables
         * from both the retrieved and tranformed data */
        ...
        /* Check if a coordinate variable was found in the retrieved data. */

        if (!ret_coord_var) {

            /* BDE:
             * I need to think about this logic a bit more carefully but
             * for now it should at least prevent bad things from happening */
        ...
        if (!trans_coord_var) {
            ...
            if (trans_dim) {
                ...
                /* If the dimension already exists this means a variable using
                 * this dimension has already been added to the coordinate
                 * system but it didn't have a coordinate variable. */
 ...
 * variables that would not be read in otherwise. */
         ...
         * trans_coord_var.
         *
         * This is needed if variables from the same datastream using the same
         * coordinate variable(s) are transformed into different coordinate
         * systems that have different units for the coordinate variables.
         */
        ...
        if (status > 0) {
            ...
            WARNING( DSPROC_LIB_NAME,
                ...
                " - trans_coord_var: %s->%s\n"
                " - units:           %s\n",
                ret_ds_group->name, ret_coord_var->name,
        ...
        param_length = 128;
        snprintf(param_name, 128, "%s:transform", trans_coord_var->name);
         ...
         * coordinate variable in the transformation coordinate system
         * to determine if a transformation is required for this dimension. */
...
//    }
    ...
    if (do_transform) {
        ...
        /* Copy the variable definition to the transformation group. */
        ...
        *trans_var = _dsproc_copy_ret_var_to_trans_group(
        ...
        trans_qc_var = _dsproc_create_trans_qc_var(trans_obs_group, *trans_var);
