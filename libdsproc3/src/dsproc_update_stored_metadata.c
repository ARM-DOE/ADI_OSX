...
*    $Revision: 57303 $
*    $Author: ermold $
*    $Date: 2014-10-06 20:11:28 +0000 (Mon, 06 Oct 2014) $
*
********************************************************************************
*
...
static InDSAttNode *_dsproc_parse_input_datastreams_att(
    ...
    InDSAttNode *root_node)
{
    ...
    while (linep) {
        ...
        if (!node) {
            ...
            if (!node) return((InDSAttNode *)NULL);
    ...
    InDSAttNode *root_node)
{
    ...
    for (node = root_node; node; node = node->next) {
        ...
        _dsproc_create_timestamp(node->start, start);

        if (node->end) {

            _dsproc_create_timestamp(node->end, end);
...
int _dsproc_update_stored_metadata(CDSGroup *dataset, int ncid)
{
    ...
    root_node = _dsproc_parse_input_datastreams_att(nc_att_value, NULL);
    if (!root_node) {
        free(nc_att_value);
        return(0);
    }
