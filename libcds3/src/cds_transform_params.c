...
/**
 *  PRIVATE: Parse a transformation parameter token out of the file buffer.
 *
 *  @param  file      - file name to use in error messages
 *  @param  cp        - pointer to the current location in the buffer
 ...
 *  @return
 *    -  1 if successful
 *    -  0 if the end of the buffer was reached before the token was found
    ...
    char **endp)
{
    ...
    *token = cp;
    ...
    if (*cp == delim) {
        ...
        ERROR( CDS_LIB_NAME,
            "Invalid format on line %d in transformation parameters file: %s\n"
    ...
    if (!(*cp) && delim != '\n') {
        ...
        ERROR( CDS_LIB_NAME,
            "Invalid format on line %d in transformation parameters file: %s\n"
            " -> expected delimiter '%c' but found end-of-file\n",
            *nlines, file, delim);
    ...
    const char *file)
{
    ...
    while (*cp) {
        ...
        status = _cds_get_token(
            full_path, cp, ':', "=;", &nlines, &obj_name, &cp);
        ...
        status = _cds_get_token(
            full_path, cp, '=', ";", &nlines, &param_name, &cp);
        ...
        status = _cds_get_token(
            full_path, cp, ';', NULL, &nlines, &param_value, &cp);
