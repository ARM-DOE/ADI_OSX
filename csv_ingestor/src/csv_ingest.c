...
*    $Revision: 66395 $
*    $Author: ermold $
*    $Date: 2015-12-10 23:53:00 +0000 (Thu, 10 Dec 2015) $
*
********************************************************************************
*
...
static char *gVersion = "$State: ingest-csv_ingestor-1.0 $";
    ...
    **************************************************************/
    ...
    for (dsi = 0; dsi < ndsids; ++dsi) {
        ...
        if (*level == '0') {
            ...
            dsp[data->ndsp] = csv_ingest_init_dsdata(name, level);
...
void csv_ingest_finish(void *user_data)
{
    ...
    if (data) {
        ...
        if (data->dsp) {
            ...
            for (dsi = 0; dsi < data->ndsp; ++dsi) {
                csv_ingest_free_dsdata(data->dsp[dsi]);
            }
    ...
    const char *file_name)
{
    ...
    *************************************************************/
    ...
    nrecs_loaded = csv_ingest_read_data(data, ds);
    ...
    *************************************************************/
    ...
    nrecs_stored = csv_ingest_store_data(data, ds);
