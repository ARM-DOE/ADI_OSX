...
int trans_bin_average_interface(interface_s is)
{
  ...
  if (cds_get_transform_param_by_dim(outvar, outvar->dims[d],
				     "missing_value", CDS_DOUBLE,
				     &_one, &missing_value) != NULL) {
  ...
  // The order is bad sdev, indeterminate sdev, bad coverage, ind coverage
  // The first two are maxima, the latter are minima (on [0,1])
  double limits[] = {HUGE, HUGE, -1, -1};
  double sdev_bad_max, sdev_ind_max, goodfrac_bad_min, goodfrac_ind_min;

  if (cds_get_transform_param_by_dim(outvar, outvar->dims[d],
				     "sdev_bad_max", CDS_DOUBLE,
				     &_one, &sdev_bad_max) != NULL) {
    limits[0]=sdev_bad_max;
  }

  if (cds_get_transform_param_by_dim(outvar, outvar->dims[d],
				     "sdev_ind_max", CDS_DOUBLE,
				     &_one, &sdev_ind_max) != NULL) {
    limits[1]=sdev_ind_max;
  }
...
int bin_average(core_s cs) {
  ...
  double sdev_bad_max = limits[0];
  double sdev_ind_max = limits[1];
  ...
  if (index_start[0] < index_start[1] && (nt == 1 || target_start[0] < target_start[1])) {
    sign = 1;
  } else if (index_start[0] > index_start[1] && (nt == 1 || target_start[0] > target_start[1])) {
    sign = -1;
  } else {
  ...
  for (j=0; j<nt; j++) {
	...
	LOG(TRANS_LIB_NAME,
	    "Standard deviation cannot be calculated: s0s2-s1s1 = %f\n", stdev[j]);
	stdev[j] = output_missing_value;
      } else {
    ...
    if (stdev[j] != output_missing_value) {
      if (stdev[j] > sdev_bad_max) {
	qc_set(qc_output[j], QC_BAD_SDEV);
      } else if (stdev[j] > sdev_ind_max) {
	qc_set(qc_output[j], QC_INDETERMINATE_SDEV);
      }
    }
