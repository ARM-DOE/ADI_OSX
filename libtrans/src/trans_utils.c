...
int get_bin_edges(double **front_edge, double **back_edge, double *index,
  ...
  *back_edge=CALLOC(nbins, double);
  ...
  if ((width=cds_get_transform_param_by_dim(var, var->dims[d],
    ...
    if (width_len != 1 && width_len != nbins) {
      ERROR(TRANS_LIB_NAME, 
  ...
  // This means we have to infer our bin edges - 
...
int set_estimated_bin_qc(int *qc_odata, CDSVar *invar, CDSVar *outvar, int d, int nt) {
  char *val;
  char key[30];
  int qc_bin=0;
  int i;
